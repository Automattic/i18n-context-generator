...
RSpec.describe I18nContextGenerator::Writers::AndroidXmlWriter do
  ...
  describe 'section header preservation' do
    it 'preserves non-managed comments (section headers)' do
      Dir.mktmpdir do |dir|
        path = File.join(dir, 'strings.xml')
        File.write(path, <<~XML)
          <resources>
    ...
    it 'updates existing managed comments in place' do
      Dir.mktmpdir do |dir|
        path = File.join(dir, 'strings.xml')
        File.write(path, <<~XML)
          <resources>
  ...
  describe 'context_mode' do
    ...
    it 'appends to non-managed comments in append mode' do
      Dir.mktmpdir do |dir|
        path = File.join(dir, 'strings.xml')
        File.write(path, <<~XML)
          <resources>
        ...
        # Non-managed comment is not ours, so we insert a new comment
