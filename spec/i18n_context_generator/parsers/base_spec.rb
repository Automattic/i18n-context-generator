...
RSpec.describe I18nContextGenerator::Parsers::Base do
  describe '.for' do
    it 'selects the JSON parser for .json files' do
      expect(described_class.for('config/translations.json')).to be_a(I18nContextGenerator::Parsers::JsonParser)
    end

    it 'selects the YAML parser for .yml and .yaml files' do
      expect(described_class.for('config/translations.yml')).to be_a(I18nContextGenerator::Parsers::YamlParser)
    ...
    it 'rejects unsupported XML files' do
      expect { described_class.for('android/res/layout/activity_main.xml') }
        .to raise_error(I18nContextGenerator::Error, /Unsupported XML format/)
    end

    it 'rejects unsupported file formats' do
      expect { described_class.for('translations.toml') }
        .to raise_error(I18nContextGenerator::Error, /Unsupported translation file format/)
    end
  end
